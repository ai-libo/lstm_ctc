...
def sparse_tuple_from(sequences, dtype=np.int32):
    ...
    shape = np.asarray([len(sequences), np.asarray(indices).max(0)[1] + 1], dtype=np.int64) #[200,20]
...
def get_data_set(dirname, start_index=None, end_index=None):
    ...
    seq_len = np.ones(inputs.shape[0]) * common.OUTPUT_SHAPE[1] #arrary([256,...,256]), shape:(64,)
